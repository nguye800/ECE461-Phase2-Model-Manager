...
def _load_json_body(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Supports both:
      ...
      - body already parsed as a dict
    """
    raw_body = event.get("body")

    # No body at all
    if raw_body is None or raw_body == "":
        raise UploadError("Request body must be valid JSON.")

    # If API Gateway has already parsed JSON for us
