...
HF_MODELS_API = os.getenv("HF_MODELS_API", "https://huggingface.co/api/models")
...
def _build_lineage_graph_payload(table, model_item: Dict[str, Any]) -> Tuple[Dict[str, Any], Set[str]]:
    ...
    def _ensure_node(
        ...
        if entry is None:
            ...
            nodes[node_id] = entry
        elif metadata and "metadata" not in entry:
            entry["metadata"] = metadata
    ...
    _ensure_node(
        artifact_id,
        model_item.get("name") or model_item.get("metadata", {}).get("name"),
        "registry",
    )

    base_models = list(model_item.get("base_models") or [])
    base_models = _augment_base_models_with_hints(base_models, model_item)
    ...
    for index, base in enumerate(base_models):
        ...
        parent_record_id = base.get("artifact_id") or base.get("model_id")
        if parent_record_id:
            related.add(parent_record_id)
        _register_parent_id(base_id)

    children_map = _collect_children_for_parents(table, set(normalized_parent_ids.keys()))
...
def _refresh_lineage_for_models(table, model_ids: Set[str]) -> None:
    refreshed: Set[str] = set()
    for model_id in sorted(model_ids):
        if not model_id or model_id in refreshed:
            continue
        key = _build_dynamo_key("model", model_id)
        existing = _fetch_existing_item(table, key)
        if not existing:
            continue
        graph, _ = _build_lineage_graph_payload(table, existing)
        existing["lineage"] = graph
        existing["updated_at"] = datetime.now(timezone.utc).isoformat()
        try:
            _put_item(table, existing)
        except ClientError as exc:
            LOGGER.warning("Failed to refresh lineage for %s: %s", model_id, exc)
            continue
...
def _collect_children_for_parents(table, parent_keys: Set[str]) -> Dict[str, List[Dict[str, Any]]]:
    if not parent_keys:
        return {}

    sk_field = os.getenv("RATE_SK_FIELD", "sk")
    meta_sk = os.getenv("RATE_META_SK", "META")
    filter_expression = Attr(sk_field).eq(meta_sk) & Attr("type").eq("MODEL")
    ...
    while True:
        ...
        for item in response.get("Items", []):
            matches = _normalized_parents_from_model(item) & parent_keys
            if not matches:
                continue
            child_entry = _simplify_child_entry(item)
            for key in matches:
...
def _normalized_parents_from_model(model_item: Dict[str, Any]) -> Set[str]:
    ...
    for base in model_item.get("base_models") or []:
        key = _normalize_lineage_key(base) or base.get("model_id")
        norm = _normalize_lineage_value(key)
        if norm:
            normalized.add(norm)
...
def handle_upload(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if artifact_type == "model":
            graph, related_lineage_ids = _build_lineage_graph_payload(table, item)
            item["lineage"] = graph
